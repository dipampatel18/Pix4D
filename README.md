...
If you wish to try out the Testudo model, you can [Download](https://umd.box.com/s/x9ytk5jfs4tbc5ichovdxfguuj8l8252) the Demo Video, Images and Model and use them on the Pix4D trial version. In case you are loading the model in the software, the software would need the source images to properly load all model, so it is advised to download all the images and provide the software with the path of those images whenever prompted. More technical details about the processing can be found in the Report.
